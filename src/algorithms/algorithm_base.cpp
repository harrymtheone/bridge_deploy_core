    ...
    const ControlConfig& control_config)
{
    ...
    for (size_t i = 0; i < num_input_nodes; i++) {
        auto input_name = ort_session_->GetInputNameAllocated(i, allocator_);
        input_name_strings_.push_back(input_name.get());
        input_names_.push_back(input_name_strings_.back().c_str());
        RCLCPP_DEBUG(node_->get_logger(), "Input %zu: %s", i, input_name_strings_.back().c_str());
    }
    
    output_name_strings_.reserve(num_output_nodes);
    output_names_.reserve(num_output_nodes);
    for (size_t i = 0; i < num_output_nodes; i++) {
        auto output_name = ort_session_->GetOutputNameAllocated(i, allocator_);
        output_name_strings_.push_back(output_name.get());
        output_names_.push_back(output_name_strings_.back().c_str());
        RCLCPP_DEBUG(node_->get_logger(), "Output %zu: %s", i, output_name_strings_.back().c_str());
    }
...
std::vector<float> AlgorithmBase::computeTargetDofPos() {
    // Clip and scale actions
    for (size_t i = 0; i < obs_.actions.size(); ++i) {
        obs_.actions[i] = std::clamp(obs_.actions[i], clip_actions_lower_, clip_actions_upper_);
        obs_.actions[i] *= config_.action_scale;
    }
