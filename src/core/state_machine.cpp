...
StateMachine::StateMachine(const rclcpp::Logger &logger)
    : current_state_(State::IDLE), pending_command_state_(State::IDLE), transition_progress_(0.0f),
      transition_duration_(2.5f), logger_(logger) {
...
bool StateMachine::transition(State new_state) {
    ...
    RCLCPP_INFO(logger_, "State: %s -> %s", stateToString(old_state).c_str(),
...
void StateMachine::onTransition(TransitionCallback callback) { transition_callback_ = callback; }
