...
RLController::RLController(const rclcpp::Node::SharedPtr &node,
                           std::shared_ptr<RobotInterface> robot_interface,
                           std::shared_ptr<AlgorithmInterface> algorithm, const Config &config)
    ...
    state_machine_->onStateEntry(
        State::STANDING_UP, std::bind(&RLController::onStandingUp, this, std::placeholders::_1));
    ...
    state_machine_->onStateEntry(
        State::RL_RUNNING, std::bind(&RLController::onRLRunning, this, std::placeholders::_1));
    state_machine_->onStateEntry(
        State::SITTING_DOWN, std::bind(&RLController::onSittingDown, this, std::placeholders::_1));
...
void RLController::start() {
    ...
    control_timer_ = node_->create_wall_timer(std::chrono::duration<double>(control_dt),
...
void RLController::rlLoop() {
    ...
    // 1. Set all joints to default positions first (safeguard for non-activated
    // joints)
    current_command_.motor.q = config_.control.default_dof_pos;

    // 2. Apply RL targets for activated joints
    // Note: target_positions from algorithm already includes default_pos + action
    for (int dof_idx : config_.robot.dof_activated_ids) {
...
bool RLController::checkSafetyLimits(const RobotState &robot_state, const RobotCommand &command) {
    ...
    float lean_angle =
        std::max(std::abs(robot_state.imu.euler[0]), std::abs(robot_state.imu.euler[1]));

    if (lean_angle > max_lean_rad) {
        RCLCPP_WARN(rclcpp::get_logger("Safety Check"),
                    "lean angle %.1f deg exceeds threshold %.1f deg", lean_angle / deg_to_rad,
                    config_.safety.max_lean_angle_deg);
        return false;
    }
        ...
        for (size_t i = 0; i < num_dof; ++i) {
            ...
            if (std::abs(tau) > tau_limit) {
                std::string joint_name = (i < config_.robot.joint_names.size())
                                             ? config_.robot.joint_names[i]
                                             : "unknown";
                RCLCPP_WARN(rclcpp::get_logger("Safety Check"),
                            "joint %zu (%s) torque %.1f Nm exceeds limit %.1f Nm", i,
                            joint_name.c_str(), tau, tau_limit);
                return false;
            }
        }
    }
