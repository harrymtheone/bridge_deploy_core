...
void RLController::computeStandUpCommand() {
    ...
    // Create a set of activated indices for fast lookup
    std::vector<bool> is_activated(current_command_.motor.q.size(), false);
    for (int dof_idx : config_.robot.dof_activated) {
        if (dof_idx >= 0 && static_cast<size_t>(dof_idx) < is_activated.size()) {
            is_activated[dof_idx] = true;
        }
    }

    for (size_t i = 0; i < current_command_.motor.q.size() && i < start_positions_.size(); ++i) {
        float default_pos = (i < config_.control.default_dof_pos.size()) ? 
                           config_.control.default_dof_pos[i] : 0.0f;
        
        if (is_activated[i]) {
            // Interpolate activated joints
            current_command_.motor.q[i] = (1.0f - progress) * start_positions_[i] + 
                                          progress * default_pos;
        } else {
            // Force non-activated joints to default position immediately
            current_command_.motor.q[i] = default_pos;
        }
    }
}
